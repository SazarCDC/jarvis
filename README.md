...
- Полностью оффлайн voice mode: wake word через `Picovoice Porcupine` (кастомный `.ppn` для «джарвис»), STT команд через `faster-whisper`, TTS через `pyttsx3`.
...
- `jarvis_assistant/voice.py` — оффлайн voice pipeline (Porcupine `.ppn` + PvRecorder + faster-whisper + TTS).
